...
def generate_question(api_key: str, model_name: str, edu_level: str, topic: str, difficulty: str, question_type: str, is_reroll: bool = False, current_question: str = "", current_code: str = "", question_index: int = 1) -> dict:
    ...
    if is_reroll:
        ...
        if question_type == "一般幾何 (平面/複合圖形)":
            prompt = f"""
            請根據主題：【{topic}】，生成一道【{difficulty}】難度的題型。
            {base_rules}
            請回傳 JSON：
            1. "question_text": 包含題目、四個選項與解析。文字中若出現「如圖」，則必須給出 python_code。
            2. "python_code": 
               【⚠️ 請根據你出的題型，嚴格選擇以下「唯一」一種對應的繪圖架構，絕對禁止混用】：
               
               - 情況一：若本題為「不等式圖解(數線)」
                 絕對禁止將解集與數線重疊！請強制照抄以下「架高式數線」程式碼：
                 # ans = 邊界點數字 (整數)
               ...
               - 情況二：若為「純幾何圖形」(如三角形、圓形，無坐標軸)
                 請使用 `ax.set_aspect('equal')` 與 `ax.axis('off')`。
                 
               - 情況三：若為「直角坐標平面」(如線形函數)
                 【完美無刻度坐標系防呆】：請絕對照抄以下畫法，加在程式碼最後面：
                 ax.spines['top'].set_visible(False)
                 ax.spines['right'].set_visible(False)
                 ax.spines['bottom'].set_position('zero')
